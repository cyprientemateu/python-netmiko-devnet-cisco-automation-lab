...
def diff_configs(desired, actual):
    ...
    for intf, config in desired.items():
        ...
        if "description" in config:
            expected_lines.append(f" description {config['description']}")

        if "ip" in config:
            expected_lines.append(" no switchport")
            expected_lines.append(f" ip address {config['ip']} {config['mask']}")
...
def main():
    ...
    for device in devices:
        ...
        # BACKUP (LOCAL)
        # -------------------------
        backup_file = f"backups/backup_{device['host']}_{timestamp}.txt"
        ...
        # -------------------------
        print("\nRunning diff engine...")
        diff_report = diff_configs(desired_config, actual)
        ...
        with open(json_file, "w") as f:
            json.dump({
                ...
                "diff": {k: str(v) for k, v in diff_report.items()}
            }, f, indent=4)

        generate_html_report(compliance, diff_report, html_file)
